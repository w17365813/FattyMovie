...
- (void)setupContentView {
    ...
    // 20 左右 R  模糊图片
    [imageView setImageToBlur:imageView.image blurRadius:100 completionBlock:nil];
    ...
    if ([BmobUser getCurrentUser]) {
        [icon sd_setImageWithURL:[NSURL URLWithString:_urlString] placeholderImage:[UIImage imageNamed:@"Image_head"]];
    }else{
    
        icon.image = [UIImage imageNamed:@"Image_head"];
    }
    ...
    [imageView sd_setImageWithURL:[NSURL URLWithString:self.urlString] placeholderImage:[UIImage imageNamed:@"1.jpg"] completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
        // 20 左右 R  模糊图片
        [imageView setImageToBlur:imageView.image blurRadius:100 completionBlock:nil];
    }];
    ...
    if ([BmobUser getCurrentUser]) {
        ...
        name.text = (self.name ? self.name : @"点击登录");}
